...
{	
	...
	*/
	...
	index_round = (int) floor(index); //the current index is rounded to a valid index
	...
	index = fmod(index, (float) SINE_TABLE_SIZE); //if index is equal or greater to 256 it must be wrapped around, as index is not necessarily an integer, the fmod function is employed
